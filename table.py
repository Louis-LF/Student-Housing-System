...
def delete_record():
    selected = table.focus()
    if LeaseNumber_entry.get() and HallName_entry.get() and HallNumber_entry.get() and RoomNumber_entry.get() and StudentName_entry.get() and OccupancyStatus_entry.get() and CleaningStatus_entry.get():
        table.item(selected, text="", values=(LeaseNumber_entry.get(),
                                            HallName_entry.get(), HallNumber_entry.get(),
                                            RoomNumber_entry.get(), StudentName_entry.get(),
                                            OccupancyStatus_entry.get(), CleaningStatus_entry.get(),RentRate_entry.get()))
...
def add_record():
    if HallNumber_entry.get() and RoomNumber_entry.get() and StudentName_entry.get() and OccupancyStatus_entry.get() and CleaningStatus_entry.get() and RentRate_entry.get():
        table.insert(parent='', index='end', text='',
                     values=(LeaseNumber_entry.get(), HallName_entry.get(),
                             HallNumber_entry.get(), RoomNumber_entry.get(), StudentName_entry.get(),
                             OccupancyStatus_entry.get(), CleaningStatus_entry.get(), RentRate_entry.get()))
        LNum = LeaseNumber_entry.get()
        HNumber = HallNumber_entry.get()
        RNumber = RoomNumber_entry.get()
        ...
        z = str(Rrate)
        Rrate = '£' + z
        sql = "insert into room (roomid,occupancy,cleaning,hallid) values (%s,%s,%s,%s)"
        mycursor.execute(sql, (RNumber,OStatus,Cstatus,HNumber,))
        mysqldb.commit()
        sql = "insert into lease (leasenumber, roomid, hallid, RentRate) values (%s,%s,%s,%s)"
        mycursor.execute(sql, (LNum,RNumber,HNumber,Rrate,))
        mysqldb.commit()
        sql = "insert into student (name,leasenumber) values (%s,%s)"
        mycursor.execute(sql, (SName,LNum))
        mysqldb.commit()
        ...
        CleaningStatus_entry.delete(0, END)
        RentRate_entry.delete(0,END)
        messagebox.showinfo(title='New Record', message='New Record Added.')
        for row in table.get_children():
            table.delete(row)
        sql = mycursor.execute("select roomid from room")
        results = mycursor.fetchall()
        ...
        messagebox.showwarning(title='Empty Record',
                               message='Can not add empty record')
...
add_button = Button(buttons_frame, text="Add Record", command=add_record)
